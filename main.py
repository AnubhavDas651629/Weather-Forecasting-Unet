...
def kl_divergence_from_probs(pred_probs, target, eps=1e-7):
    ...
    kl = target * (target.log() - pred_probs.log()) + (1.0 - target) * (
        (1.0 - target).log() - (1.0 - pred_probs).log()
    )
    return kl.mean()
...
if __name__ == "__main__":
    ...
    if len(all_files) == 0:
        raise RuntimeError(
            f"No 2014 .npy files found in {cape_dir}. Run preprocess.py first."
        )
    ...
    print(
        f"2014 days found: {len(all_files)}  |  "
    ...
    # ── Loss: Weighted BCE ───────────────────────────────────────────────────
    # Extreme class imbalance (tornadoes are very rare) means standard BCE or
    # unweighted KL divergence will just predict 0 everywhere.
    # We heavily weight the positive (tornado) pixels to force the model to predict them.
    ...
    for epoch in range(EPOCHS):
        ...
        for x, y in tqdm(train_dataloader, desc=f"Train {epoch + 1}/{EPOCHS}"):
            ...
            optimizer.zero_grad()
            logits = model(x)
            # Primary loss: Weighted BCE to overcome extreme imbalance
            loss = bce_criterion(logits, y)
            loss.backward()
            optimizer.step()
            train_bce_total += loss.item()

            # Secondary metric: KL divergence
            with torch.no_grad():
                train_kl_total += kl_divergence_from_logits(logits, y).item()
